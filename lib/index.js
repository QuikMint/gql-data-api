        ...
        async contracts(client) {
            try {
                const contracts = await admin
                    ...
                    .get();
                return contracts.docs.map(contract => contract.data());
            }
            catch (error) {
                throw new apollo_server_1.ApolloError(error);
            }
        },
        async customers(client) {
            try {
                const customers = await admin
                    ...
                    .where('client_id', '==', client.id)
                    .get();
                return customers.docs.map(customer => customer.data());
            }
            catch (error) {
                throw new apollo_server_1.ApolloError(error);
            }
        ...
        async transactions(client) {
            try {
                const transactions = await admin
                    ...
                    .where('client_id', '==', client.id)
                    .get();
                return transactions.docs.map(transaction => transaction.data());
            }
            catch (error) {
                throw new apollo_server_1.ApolloError(error);
            }
        ...
        async transactions(customer) {
            try {
                const transactions = await admin
                    .firestore()
                    .collection('transactions')
                    .where('customer', 'in', customer.transactions)
                    .get();
                return transactions.docs.map(transaction => transaction.data());
            }
            catch (error) {
